...
日期：2026/03/30
核心理解：
  - Function Calling 的本质是：让 LLM 不直接回答，而是返回"我需要调用某个函数来获取信息"，我在代码中执行这个函数，再把结果喂回给 LLM，由它组织最终回答
  - 完整流程的 4 个步骤是：
  1.发送请求（包含 tools 定义）
  2.返回 tool_calls（函数名 + 参数）
  3.执行函数，得到结果
  4.把结果回传给 LLM
  - arguments 字段需要注意：返回的是 JSON 格式的字符串，必须用 `json.loads()` 解析成 Python 字典后才能作为函数参数使用
  - description 的重要性体现在：LLM 根据这段描述决定何时调用此工具
与传统开发的最大差异：传统开发中我是"决策者"（代码决定调用什么），Function Calling 中 LLM 是"决策者"（LLM 决定调用什么），我是"执行者"（代码负责执行并返回结果）
遗留问题（后续查）：暂无
```
